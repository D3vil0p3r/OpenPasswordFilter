...
namespace OPFService
{
    class NetworkService
    {
        ...
        private void writeLogError(string message, System.Diagnostics.EventLogEntryType level)
        {
            using (EventLog eventLog = new EventLog("Application"))
            {
                eventLog.Source = "OpenPasswordFilter";
        ...
        private void writeLogValidating(string message, System.Diagnostics.EventLogEntryType level)
        {
            using (EventLog eventLog = new EventLog("Application"))
            {
                eventLog.Source = "OpenPasswordFilter";
                eventLog.WriteEntry(message, level, 102, 1);
            }
        }
        ...
        public void main(Socket listener)
        {
            ...
            {
                writeLogError(e.Message, EventLogEntryType.Error);
                writeLogError("Unable to bind local port", EventLogEntryType.Error);
            }
        }
        ...
        public void handle(Socket client)
        {
            try
            {
                ...
                if (command == "test")
                {
                    ...
                    writeLogValidating("Validating password for user " + username, EventLogEntryType.Information);
