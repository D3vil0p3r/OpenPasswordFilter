    ...
    class OPFGroup
    {
        ...
        private void ReadInGroupsFile()
        {
            ...
            while ((line = infilematch.ReadLine()) != null)
            {
                ...
                {
                    writeLogError("Died trying to ingest line number " + a.ToString() + " of groups file.", EventLogEntryType.Error);
                }
            }
            infilematch.Close();
            writeLog("Successfully read " + (a - 1).ToString() + " groups.", EventLogEntryType.Information);
        }
